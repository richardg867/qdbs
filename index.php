        ...
        switch ($_POST['do']) {
            case 'add':
                 ...
                 if (ini_get("magic_quotes_runtime") or ini_get("magic_quotes_gpc")) {
                      $sql = "INSERT INTO ".$_qdbs['tpfx']."queue (id,quote,ip) VALUES (NULL, '".$db->escape(stripslashes($quote))."', '".$db->escape(stripslashes($_SERVER['REMOTE_ADDR']))."')";
                 } else {
                      $sql = "INSERT INTO ".$_qdbs['tpfx']."queue (id,quote,ip) VALUES (NULL, '".$db->escape($quote)."', '".$db->escape($_SERVER['REMOTE_ADDR'])."')";
                 }
                 $r = $db->_sql($sql);
                 print($tpl->fetch($tpl->tdir.'layout_header.tpl'));
